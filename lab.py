class Flower:
    # Приватні атрибути класу для представлення характеристик квітки
    __height = None
    __size = None
    __color = None
    __price = None
    ...
    # Методи для встановлення значень приватних атрибутів
    ...
    # Конструктор класу, який ініціалізує атрибути квітки
    ...
    # Метод для представлення квітки у вигляді рядка
    def __str__(self):
...
class FlowerShop:
    def __init__(self):
        # Ініціалізуємо список для зберігання квітів у квітковому магазині
        self.list_of_flowers_in_shop = []

    # Метод для додавання квітки до магазину
    def add_flower_to_shop(self, flower):
        self.list_of_flowers_in_shop.append(flower)

    # Метод для отримання списку найдорожчих квітів
...
class Bouquet:
    ...
    # Метод для додавання квіток до букету з вказаною кількістю
    def add_flower_to_bouquet(self, flower, quantity):
        for _ in range(quantity):
            self.flowers.append(flower)

    # Метод для обчислення загальної ціни букету
    def get_total_price(self):
        return sum(flower.get_price() for flower in self.flowers)

    # Метод для представлення інформації про букет у вигляді рядка
    def __str__(self):
        return f"Bouquet - Total Price: {self.get_total_price()}"


if __name__ == "__main__":
    # Створення об'єктів квіток з різними характеристиками
    rose = Flower(20, "Medium", "Red", 5.99, 10, 1)
    tulip = Flower(15, "Small", "Yellow", 3.99, 15, 0.5)
    lily = Flower(25, "Large", "White", 6.99, 8, 2)

    # Створення об'єкта квіткового магазину та додавання квіток до нього
    ...
    # Виведення інвентаря магазину у консоль
    print("Flower Shop Inventory:")
    print(flower_shop)

    # Створення букету та додавання квіток до нього
    bouquet = Bouquet()
    bouquet.add_flower_to_bouquet(rose, 3)
    bouquet.add_flower_to_bouquet(tulip, 5)

    # Виведення деталей букету у консоль
    print(f"Bouquet Details: {bouquet}")

    # Знаходження та виведення найдорожчих квіток у магазині
